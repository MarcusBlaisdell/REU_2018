...
class pg_Class():
    ...
    validationData = []
    testData = []
    T = 3 # default value of T
    k = 100 # default value of k
    lam = 1e3 # lambda
    kmer = 17 # default k-mer size is 11
    ...
    eta = 1.0 # eta is calculated for each iteration
    trainMistakes = 0
    trainTotal = 0
    testMistakes = 0
    testTotal = 0
    ...
    for t = 1, 2, ..., T:
        ...
        set A_t+ = {i in the set of A_t: yi<w_t, x_i> < 1} ### (add incorrect predictions to subset)
    ...
    def pegasosBatch (self, trainDataSet,  t):
        ...
        for record in A_t_plus:
            ...
            ### record[0] is the index,weight array
            ### record[1] is the label
            ...
            for element in record[0]:
                if runxit.get(element[0], '--') == '--':
                    ...
                    '''
                    if record[1] == 1: # if the document is actually good:
                        newEta = 2
                    else: # if the document is actually bad:
                        newEta = 0.2
                    '''
                    #newEta = record[1] + (record[1] * self.dotProd(record[0]))
                    #runxit[element[0]] += newEta * element[1] * record[1]
                    runxit[element[0]] += element[1] * record[1]
    ...
    def testWeight (self, testDataSet, t):
        ...
        for i in range(len(testDataSet)):
            ...
            if yHat == 1: # if we predict good:
                self.testdp += 1
                if yit == 1: # if it is actually good:
                    #print 'successfully predicted Good'
                    self.tp += 1
                    self.testnpr += 1
                    self.testGood += 1
                else: # if it is actually bad:
                    self.testMistakes += 1
                    #print '     Mistakenly predicted Good'
                    self.fp += 1
            else: # if we predict bad:
                if yit == 1: # if it is actually good:
                    self.testMistakes += 1
                    self.testGood += 1
                    #print '                                   Mistakenly predicted Bad'
                    self.fn += 1
                else: # if it is actually bad:
                    #print '                             successfully predicted Bad'
                    self.tn += 1
